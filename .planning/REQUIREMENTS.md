...
- [x] **DASH-03**: Service card borders animate with light traces running along their edges
- [x] **DASH-04**: Health state components glow and pulse (slow breathing animation for healthy; sharp pulse for warning/alert)
- [x] **DASH-05**: Color system applied consistently: Tron Blue `#00c8ff` = healthy/online, Red `#ff4444` = down/critical, Amber `#ffaa00` = warning/degraded
- [x] **DASH-06**: Scrolling "Now Playing" banner displays active Plex streams (title, user, playback progress) at top or bottom of dashboard
- [x] **DASH-07**: Tapping/clicking a service card navigates to a dedicated detail view for that service with expanded metrics
- [x] **DASH-08**: All animations use `transform` and `opacity` only — no layout-triggering properties — to maintain 60fps on mobile
...
- [x] **SVCRICH-01**: Pi-hole card shows total DNS queries today, block percentage, and blocklist size; targets Pi-hole v6 API only; settings tab displays "Pi-hole v6 or higher required"
